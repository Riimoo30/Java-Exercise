...
public class Main {
    ...
    public static double exercise1(String text, double expectedScore) {
        ...
        //declaring that the score can't be above 20
       if (yourScore > 20){
            yourScore = 20;
        }
        //if the string contains the letter "a" remove 5 points
        if(text.contains("a")) {
            yourScore -= 5;
        }
        //if the string is all lower case add 10 points

        if(text.equals(text.toLowerCase())){
            yourScore += 10;
        }
        //if the score now is over 20 multiply by 2
        if(yourScore > 20){
            yourScore *= 2;
        }
